...
def sigmoid_log_double_softmax(
    ...
    scores[:, :-1, -1] = -F.softplus(z0.squeeze(-1))
    scores[:, -1, :-1] = -F.softplus(z1.squeeze(-1))
    return scores
...
class LightGlue(nn.Module):
    ...
    def __init__(self, features="superpoint", **conf) -> None:
        ...
        state_dict = None
        if features is not None or conf.weights is not None:
            path = Path(__file__).parent.parent / "weights" / f"{conf.weights}.pth"
