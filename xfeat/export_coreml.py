...
def main():
    ...
    print("Converting combined model to CoreML...")
    traced_model = torch.jit.trace(model, (dummy_image0, dummy_image1))

    mlmodel = ct.convert(
        traced_model,
        inputs=[
            ct.TensorType(name="image0", shape=dummy_image0.shape),
